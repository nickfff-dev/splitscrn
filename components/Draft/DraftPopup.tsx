...
const DraftPopup = ({players,ondraftPick, socketdata, draftPeople}:{players:any,ondraftPick:any, socketdata:any, draftPeople:any}) => {
  ...
  return (<div className="bg-gradient-to-r from-primary to-secondary p-1 rounded-xl w-[1000px] w-full  mx-auto">
    <div className="bg-gray-dark p-6 rounded-xl ">
      ...
      <div className="grid  grid-flow-row col-start-1 col-end-8 auto-rows-auto justify-center  ">
        ...
        <div className="grid grid-flow-col col-start-2 col-end-6 grid-cols-4  items-center  row-start-2 row-end-3  rounded-full bg-gray-medium space-x-12 ">
          ...
          <p className="text-gray-300 font-bold">Humanoid</p>
          <p className="text-gray-300 font-bold">$30000</p>
          <button onClick={ondraftPick(selectedPlayer)} className="rounded-full  px-4 py-1    capitalize my-auto uppercase outline outline-1  outline-secondary  text-gray-300 font-bold max-w-max">CONFIRM</button>
        ...
        <div className="grid grid-flow-col col-start-7  col-end-8 grid-cols-2  row-start-1 row-end-7 ">
          ...
          <div className="col-span-2 text-center w-48 row-start-4 row-end-7 grid  grid-flow-row bg-gray-medium rounded-xl h-[500px] ">
            <div className="row-start-1 row-end-2 h-12 py-1 text-center bg-gray-light rounded-xl"><p className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent font-bold text-3xl">${socketdata.balance.toLocaleString("en-US")}  </p></div>
...
<div className="   grid col-start-1 col-end-7   grid-cols-7 text-center ">       
          ...
          <ul className="list-decimal list-outside   col-start-1 col-end-7 grid-cols-7 space-y-2 text-white">
            ...
            draftPeople?    draftPeople.map((participant: any, index:number) => {
                return (
                  ...
                  <p className=" pl-2 text-gray-300 font-medium ">{participant ?  participant?.username.split(" ")[0] : null}</p>
    ...
    <div id="acquire" className={`${showAcquire ? "" : "hidden"} absolute top-24 z-40 left-20 right-20`}><Acquire closeAcquire={closeAcquire} players={players} fetchPlayer={fetchPlayer } /></div>
